...
impl<MutexRaw: RawMutex + 'static, const N: usize> SystemController<MutexRaw, N> {
    ...
    pub fn handle_command(&mut self, cmd: SystemCommand) {
        match cmd {
            SystemCommand::Wake => {
                if !self.battery_critical && !self.thermal_critical && self.status != SystemStatus::Active {
            ...
            SystemCommand::Sleep => {
                let can_sleep = self.time_in_active >= 30
            ...
            SystemCommand::BatteryUpdate {
                ...
                if state_of_charge < 10 && !charging {
                    ...
                    let _ = self.led_tx.try_send(SystemLedState::BlinksRedOncePerThirtySeconds);
                } else {
