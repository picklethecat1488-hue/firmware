...
impl<I: I2c> Max17048<I> {
    ...
    pub async fn init(&mut self) -> Result<(), PeripheralError> {
        ...
        if (status & 0x0100) != 0 {
            ...
            self.write_register(Register::MODE, mode | 0x4000).await?;
        }
        Ok(())
    }
...
impl<I: I2c> FuelGauge for Max17048<I> {
    ...
    async fn read_voltage_mv(&mut self) -> Result<u32, Self::Error> {
        let res = self.read_register(Register::VCELL).await;
    ...
    async fn read_state_of_charge(&mut self) -> Result<u8, Self::Error> {
        let res = self.read_register(Register::SOC).await;
        if let Err(ref _e) = res {
            log_warn!(
    ...
    async fn configure_alerts(
        ...
        self.write_register(Register::VALRT, valrt_word).await?;
    ...
    async fn check_and_clear_alerts(&mut self) -> Result<(bool, bool), Self::Error> {
        ...
        if new_status != status {
            self.write_register(Register::STATUS, new_status).await?;
        }
...
impl<I: I2c> Tickable for Max17048<I> {
    ...
    async fn tick(&mut self) -> Result<(), Self::Error> {
        let voltage = self.read_voltage_mv().await?;
        let soc = self.read_state_of_charge().await?;
...
impl<I: I2c> ChargeStatus for Max17048<I> {
    ...
    #[tracing::instrument(level = "trace")]
    async fn get_charge_state(&mut self) -> Result<ChargeState, Self::Error> {
        let crate_val = self.read_register(Register::CRATE).await? as i16;
        let status = self.read_register(Register::STATUS).await?;
...
impl<I: I2c> Probeable for Max17048<I> {
    ...
    async fn read_chip_id(&mut self) -> Result<u16, Self::Error> {
        let id = self.read_register(Register::VRESET).await?;
    ...
    async fn reset(&mut self) -> Result<(), Self::Error> {
        #[cfg(all(target_arch = "arm", target_os = "none"))]
        {
            ...
            let _ = self.write_register(Register::CMD, 0x5400).await;
            ::embassy_time::Timer::after_millis(15).await; // Wait for reset (datasheet: 15ms)
            self.read_chip_id().await.map(|_| ())
        }
        #[cfg(not(all(target_arch = "arm", target_os = "none")))]
        {
            self.write_register(Register::CMD, 0x5400).await
        }
    }
}
